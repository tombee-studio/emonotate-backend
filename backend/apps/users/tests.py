...
class CurveWithYouTubeSerializerTest(TestCase):
    ...
    def test_serialize_curve(self):
        ...
        json = JSONRenderer().render(serializer.data).decode()
...
class RequestAPITestCase(APITestCase):
    ...
    def test_api_superuser(self):
        ...
        response = client.post(f"{url}?format=json", json.dumps({
            "title": Faker().word(),
            "description": Faker().sentence(),
            "intervals": random.randint(1, 20),
            "owner": EmailUser.objects.get(username="tomoya").id,
        ...
        req_id = json.loads(response.content.decode())['id']
        ...
        self.assertEqual(response.status_code, 201)
        response = client.put(url + f"{req_id}/", json.dumps({
            "title": Faker().word(),
            "description": Faker().sentence(),
            "intervals": random.randint(1, 20),
            "owner": EmailUser.objects.get(username="tomoya").id,
